function updateDifficulty(levelWon)
{

/*
YK
saves the level that you acheaved.
*/
if(typeof(Storage)!=="undefined")
  {
  
  //localStorage.dificulty_level=0;
  if (localStorage.dificulty_level)
  {
	if(levelWon > localStorage.dificulty_level){
		localStorage.dificulty_level = levelWon;
  }
    ...
    {
    localStorage.dificulty_level=1;
    }
  document.getElementById("reached_difficulty_lvl").innerHTML="You passed level: <strong>" + localStorage.dificulty_level + "</strong>! ";
  }
else
  {
  document.getElementById("reached_difficulty_lvl").innerHTML="Sorry, your browser does not support web storage... so cant keep track of wins";
  }
}



function updateDifficultyNav(lvlCtrl){
/*
YK:
This just to unlock the level navigation. Takes the passed level from local web storage.

*/

if(typeof(Storage)!=="undefined")
  {
  //localStorage.dificulty_level=0; //debug reset.
  if (localStorage.dificulty_level)
    {
		var i;
		var thisButton;
		//Show only unlocked levels. + 1 is where the new level is revealed
		for (i = 0; i < Number(localStorage.dificulty_level) +1 ; i = i + 1) {
			
			thisButton = lvlCtrl.children[i];
			thisButton.style.display = "initial";
  ...
  {
  //Local save failed, just display all levels for this device.
  	for (i = 0; i < lvlCtrl.children.length ; i = i + 1) {
...
var Level = function (evt, rows, cols, matches) {
    "use strict";
  //  var cardsList           = '!"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVW[\\]^_`abcdefghijklmnopqrstuvwxyzÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜ', As removed because of repeat card bug.. probably character error
    var cardsList           = '!"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVW[\\]^_`abcdefghijklmnopqrstuvwxyz',
    ...
    var lvlNum      = 0,
		...
		start = function () {
            ...
            currentLvl.onwin = function (clicks, prc) {
				...
				updateDifficulty(Number(lvlNum) + 1);
